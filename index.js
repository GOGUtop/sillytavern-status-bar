(function () {
  ...
  function init(){ getSettings(); ensurePanel(); const button=$('<div id="sbm_open" class="fa-solid fa-bars" title="状态栏管理器"></div>'); $('#extensions_settings2').append(button); button.on('click',()=>{ensurePanel();$('#status_bar_manager_panel').toggle();}); if(window.eventSource&&window.event_types){eventSource.on(event_types.MESSAGE_RECEIVED,generateBars);} }
